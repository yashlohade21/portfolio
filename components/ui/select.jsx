...
function SelectTrigger({ className, children, ref, ...props }) {
  return (
    <SelectPrimitive.Trigger
      ref={ref}
      className={cn(
        "flex h-[48px] w-full items-center justify-between rounded-md border border-[var(--glass-border)] bg-[var(--glass-bg)] px-4 py-5 text-base text-[var(--text-secondary)] placeholder:text-[var(--text-muted)] focus:border-accent outline-none transition-all duration-300",
        className
      )}
      {...props}>
      {children}
